...
- Solo play against Easy, Standard, or Expert AI; local pass-and-play; persistent asynchronous friend boards.
...
Open two private browser windows. Each tab gets a simulated player, so you can find the other player under Rivals, send an asynchronous board, open it from the recipient’s lobby, and play moves. The recipient is White and always moves first. This uses the local authoritative room server; real RUN identity and OS push delivery still require the host.
